...
class SurveyPage extends Component {
  ...
  gotoDetail = (surveyID, surveyName) => { 
    ...
    this.props.dispatch({type: 'SET_DATA', data: {surveyId: surveyID, surveyName: surveyName} })
  }
  ...
  render() {
    ...
    for (let i = 0; i < this.state.surveys.length; i+=2)      
    {
      ...
      for (let j = 0; j < 2 && i + j < this.state.surveys.length; j++) {
        ...
        let campaigns = []
        rowItems.push( 
          <div className="col-md-6" key={j} onClick={()=>this.gotoDetail(survey.id, survey.name)} >
            <div className="cont">
